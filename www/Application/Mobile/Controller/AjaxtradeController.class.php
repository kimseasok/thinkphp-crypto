...
class AjaxtradeController extends MobileController
{
    ...
    public function getcoinprice($symbol = null)
    {
        ...
        $fakes = array("CIX/USDT", "XID/USDT", "TIX/USDT");

        if (in_array($symbol, $fakes)) {
            $symbol = strtolower(str_replace('/', '_', $symbol));
    ...
    public function gettradbuy($symbol = null)
    {
        ...
        if (in_array($symbol, $fakes)) {
            $market = strtolower(str_replace('/', '_', $symbol));
    ...
    //获取行情单个行情数据
    public function obtain_cix($coin)
    {
        $symbol = "cix_usdt";
        $mlist = M("market")->where(array('name' => $symbol))->field("new_price,min_price,max_price,faxingjia,volume")->find();
        $num = 0.001 * rand(1, 9);
        ...
        if ($change >= 0) {
            $change = '<span class="fzmm bgreen market-list-info">+' . $change . '%</span>';
        } else {
            $change = '<span class="fzmm bred market-list-info">' . $change . '%</span>';
        }
    ...
    public function obtain_xid($coin)
    {
        $symbol = "xid_usdt";
        $mlist = M("market")->where(array('name' => $symbol))->field("new_price,min_price,max_price,faxingjia,volume")->find();
        $num = 0.001 * rand(1, 9);
    ...
    public function obtain_tix($coin)
    {
        $symbol = "tix_usdt";
        $mlist = M("market")->where(array('name' => $symbol))->field("new_price,min_price,max_price,faxingjia,volume")->find();
        $num = 0.001 * rand(1, 9);
    ...
    public function obtain_btc($coin)
    {
        ...
        if ($data['status'] == 'ok') {
            ...
            // print_r($data);exit;
            if ($change >= 0) {
                $changestr = "<span  class='fzmm bgreen market-list-info'>+" . number_format($change, 2, '.', '') . "%</span>";
            } else {
                $changestr = "<span  class='fzmm bred market-list-info'>" . number_format($change, 2, '.', '') . "%</span>";
            }

            if ($close >= $open) {
                $close = "<span class='fzmmm fw  fe6 '>" . number_format($close, 2, '.', '') . "</span>";
            } else {
                $close = "<span  class='fzmmm fw  fe6 '>" . number_format($close, 2, '.', '') . "</span>";
            }
            $alldata['code'] = 1;
